...
public class ChangUtils {
    ...
    public enum Crypto {
        ...
        public String encryptText(String text) throws Exception {
            Cipher cipher = Cipher.getInstance(Crypto.Algorithm.get());
            SecretKeySpec keySpec = new SecretKeySpec(Crypto.Key.get().getBytes(), "AES");
            IvParameterSpec ivParamSpec = new IvParameterSpec(Crypto.IV.get().getBytes());
